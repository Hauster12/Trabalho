...
function esperarEnter(): Promise<void> {
...
async function batalha(p1: Personagem, p2: Personagem) {
    ...
    while (p1.estaVivo() && p2.estaVivo()) {
        ...
        if (acao1 === 0) p1.atacar(p2);
        else if (acao1 === 1) p1.aprimorarHabilidade();
        else if (acao1 === 2) p1.regenerarVida();

        if (!p2.estaVivo()) break; 

        // Ação do Personagem 2
        if (acao2 === 0) p2.atacar(p1);
        else if (acao2 === 1) p2.aprimorarHabilidade();
        else if (acao2 === 2) p2.regenerarVida();
    ...
    if (p1.estaVivo() && !p2.estaVivo()) console.log(`\n${p1.nome} venceu!`);
    else if (p2.estaVivo() && !p1.estaVivo()) console.log(`\n${p2.nome} venceu!`);
    else console.log(`\nAmbos caíram!`);
    rl.close();
}
